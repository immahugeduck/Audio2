...
export const UrlAudioAnalyzer: React.FC<UrlAudioAnalyzerProps> = ({
  ...
  const handleAnalyzeUrl = async (targetUrl?: string) => {
    ...
    if (ytId) {
      ...
      //
      // HONESTY NOTE: we only embed the video player here. We do NOT touch the
      // audio engine or spectrum analyzer for YouTube links, because there is
      // no legitimate client-side way to pull the real audio out of a YouTube
      // stream - YouTube blocks that, and a server-side extractor runs into
      // their Terms of Service. This used to silently swap in a canned
      // synthwave sample and label it "Real-Time FFT Analyzer Sync: ACTIVE",
      // which made the spectrum analyzer show fake data while claiming it was
      // analyzing the video. The UI below is upfront instead: watch the video
      // here, and use Mic/System Loopback if you want the analyzer to see the
      // video's actual audio.
